        ...
        for ite in range(self.T):
            ...
            while True: # for each iteration of RBS, keep search until it converges
                ...
                for i in range(len(knob_names)):
                    ...
                    current_granularity.append((current_max[i]-current_min[i])/self.N)
                ...
                for i in range(self.N):
                    ...
                    for j in range(len(knob_names)):
                        knob_value.append(max(knob_min[j],current_min[j]+current_granularity[j]*sample[j][i]))
                    knob_value_record.append(knob_value)
                    db_connector.change_knob(knob_names,knob_value,knob_type)
                ...
                for i in range(len(knob_names)):
                    current_min[i]=max(knob_min[i],current_min[i]+(interval_id[i]-1)*current_granularity[i])
                    current_max[i]=min(knob_max[i],current_min[i]+(interval_id[i]+1)*current_granularity[i])
