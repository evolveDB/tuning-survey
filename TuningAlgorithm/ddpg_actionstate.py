...
class ActorCritic:
    def __init__(self, state_num,action_num, learning_rate=0.1, train_min_size=32, size_mem=2000, size_predict_mem=2000):
    ...
    def act(self, cur_state,cur_action):
        ...
        # if torch.cuda.is_available():
        #     cur_state.cuda()
        action = self.actor_model(cur_state,cur_action).cpu().detach().numpy()[0]
...
class DDPG_Algorithm():
    ...
    def take_action(self,action):
        ...
        knob_value=np.minimum(np.maximum(knob_value,self.knob_min),self.knob_max)
    ...
    def train(self,total_epoch,epoch_steps,save_epoch_interval,save_folder):
        ...
        for step in range(50):
            ...
            trial.append({'latency':latency,'throughput':throughput,'action':action,'current_state':current_state,'new_state':new_state,'reward':reward})
        ...
        for s in random_record:
            self.model.remember(self.state_scaler.transform(np.array(s[0]).reshape(1,-1))[0],s[1],s[2],self.state_scaler.transform(np.array(s[3]).reshape(1,-1))[0])
        self.logger.write("")
        for epoch in range(total_epoch): 
            ...
            for step in range(epoch_steps):
                ...
                trial.append({'latency':latency,'throughput':throughput,'current_action':current_action,'delta_action':delta_action,'current_state':current_state,'new_state':new_state,'reward':reward})
                self.logger.write("Step reward: "+str(reward)+", Epoch Total Reward: "+str(epoch_total_reward)+"\n")
            ...
            self.logger.write("Epoch "+str(epoch)+" finish, total time: "+str(epoch_end_time-start_time)+"s\n")
    ...
    def calculate_reward(self,latency,throughput):
        ...
        dt_0=(throughput-self.init_throughput)/max(self.init_throughput,throughput)
        dl_last=(self.last_latency-latency)/max(self.last_latency,latency)
        dt_last=(throughput-self.last_throughput)/max(self.last_throughput,throughput)
        r_l=0
        if dl_0>0:
