...
class PostgresExecutor(Executor):
    ...
    def change_knob(self, knob_name, knob_value, knob_type):
        ...
        for i in range(len(knob_name)):
            if knob_type[i] == "integer":
                sql = "alter database " + \
                    str(self.database) + " set " + str(knob_name[i]) + "=" + str(int(knob_value[i]))
            else:
                sql = "alter database " + \
                    str(self.database) + " set " + str(knob_name[i]) + "=" + str(knob_value[i])
            cur.execute(sql)
        cur.close()
        conn.commit()
        conn.close()
    ...
    def reset_knob(self, knob_name: list):
        ...
        for knob in knob_name:
            sql = "alter database " + \
                str(self.database) + " reset " + str(knob)
            cur.execute(sql)
        cur.close()
        conn.commit()
        conn.close()
    ...
    def get_db_state(self):
        conn = self.get_connection()
        cur = conn.cursor()
        sql = "select * from pg_stat_database where datname='" + \
            str(self.database) + "'"
        cur.execute(sql)
        result = cur.fetchall()
        state_list = []
